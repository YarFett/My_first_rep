...
function ProdDocs() {
  ...
  const [fileCopyOfCharter, setFileCopyOfCharter] = useState<File>();
  const [fileLawOfUse, setFileLawOfUse] = useState<File>();
  const [fileLicense, setFileLicense] = useState<File>();

  const [answers, setAnswer] = useContext(NewAppContext)
  const history = useNavigate();

  const handleFileChangeStatement = (e: any) => {
    const file = e.target.files[0] as Blob;
    const reader = new FileReader();
    reader.readAsArrayBuffer(file)
    reader.onload = () => {
      const newFile = new Uint8Array(reader.result as ArrayBuffer);
      setItemToLocalStorage('fileStatementData', Array.from(newFile));
      console.log(newFile)
    }
    setFileStatement(e.target.files[0])
  };
  const handleFileChangeCopyOfCharter = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const file = e.target.files[0] as Blob;
      const reader = new FileReader();
      reader.readAsArrayBuffer(file)
      reader.onload = () => {
        const newFile = new Uint8Array(reader.result as ArrayBuffer);
        setItemToLocalStorage('fileCopyOfChartertData', Array.from(newFile));
        console.log(newFile)
      }
      setFileCopyOfCharter(e.target.files[0]);
    }
  };
  const handleFileChangeLawOfUse = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const file = e.target.files[0] as Blob;
      const reader = new FileReader();
      reader.readAsArrayBuffer(file)
      reader.onload = () => {
        const newFile = new Uint8Array(reader.result as ArrayBuffer);
        setItemToLocalStorage('fileLawOfUseData', Array.from(newFile));
        console.log(newFile)
      }
      setFileLawOfUse(e.target.files[0]);
    }
  };
  const handleFileChangeLicense = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const file = e.target.files[0] as Blob;
      const reader = new FileReader();
      reader.readAsArrayBuffer(file)
      reader.onload = () => {
        const newFile = new Uint8Array(reader.result as ArrayBuffer);
        setItemToLocalStorage('fileLicenseData', Array.from(newFile));
        console.log(newFile)
      }
  ...
  const onclick = (e: any) => {
    if (fileStatement) {
      ...
      let data_fileStatement = new FormData()
      data_fileStatement.append('Statement', fileStatement)
      setItemToLocalStorage('fileStatement', fileStatement.name)
      setItemToLocalStorage('fileStatementType', fileStatement.type)
    }

    if (fileCopyOfCharter) {

      let data_fileCopyOfCharter = new FormData()
      data_fileCopyOfCharter.append('CopyOfCharter', fileCopyOfCharter)
      setItemToLocalStorage('fileCopyOfCharter', fileCopyOfCharter.name)
      setItemToLocalStorage('fileCopyOfCharterType', fileCopyOfCharter.type)
    }

    if (fileLawOfUse) {

      let data_fileLawOfUse = new FormData()
      data_fileLawOfUse.append('LawOfUse', fileLawOfUse)
      setItemToLocalStorage('fileLawOfUse', fileLawOfUse.name)
      setItemToLocalStorage('fileLawOfUseType', fileLawOfUse.type)
    }

    if (fileLicense) {

      let data_fileLicense = new FormData()
      data_fileLicense.append('License', fileLicense)
      setItemToLocalStorage('fileLicense', fileLicense.name)
      setItemToLocalStorage('fileLicenseType', fileLicense.type)
    }
        ...
        <div className="order-request__container">
          <div className="constructor">
            <header className="constructor__header">
              <a id="GoBack" className="constructor__link-back" onClick={clickBack}>Назад</a>
              {/* <Link className="constructor__link-back" to={"/cert_fstek"}>Назад</Link> */}
              <p className="constructor__title">Документы поставщика</p>
            </header>
            <div className="constructor__inner">
                ...
                <div className="cb-spoiler is-toggle">
                  <div className="cb-spoiler__field cb-spoiler__field--substrate">
                    <div className="cb-spoiler__value">
                      ...
                      <p className="cb-spoiler__input">Подтверждение полномочий лица, подписавшего заявление</p>
                    </div>
                    <div className="cb-spoiler__body">
                      ...
                      <svg width="48" height="24" viewBox="0 0 48 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <a href="#" className="constructor__file">{fileStatement && `${fileStatement.name}`}</a>
                      ...
                      <div className="cb-file-uploader">
                        <input id="fileStatement" type="file" className="cb-file-uploader__visually-hidden" onChange={handleFileChangeStatement} multiple />
                        ...
                        <label htmlFor="fileStatement" className="cb-file-uploader__label">
                ...
                <div className="cb-spoiler is-toggle">
                  <div className="cb-spoiler__field cb-spoiler__field--substrate">
                    <div className="cb-spoiler__value">
                      ...
                      <p className="cb-spoiler__input">Копия устава заявителя</p>
                    </div>
                    <div className="cb-spoiler__body">
                      <div className="constructor__row">
                        {/* <span className="cb-icon cb-icon__size-48">
                      <svg width="48" height="24" viewBox="0 0 48 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <a href="#" className="constructor__file" >{fileCopyOfCharter && `${fileCopyOfCharter.name}`}</a>
                        <span className="cb-icon cb-icon__size-24">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" clipRule="evenodd" d="M7.99782 5.33184C8.20421 3.9903 9.35852 3 10.7158 3H13.2842C14.6415 3 15.7958 3.99031 16.0022 5.33184L16.105 5.99996H17.4955H19L19 6H20V7.5H18.8833L18.0486 18.2326C17.9271 19.7945 16.6242 21 15.0576 21H8.94239C7.37576 21 6.07291 19.7945 5.95143 18.2326L5.11667 7.5H4V6H5L5 5.99996H6.50453H7.89503L7.99782 5.33184ZM14.5873 5.99996H9.41268L9.48037 5.55993C9.57419 4.95014 10.0989 4.5 10.7158 4.5H13.2842C13.9011 4.5 14.4258 4.95014 14.5196 5.55993L14.5873 5.99996ZM6.6212 7.5L7.44691 18.1163C7.50765 18.8972 8.15908 19.5 8.94239 19.5H15.0576C15.8409 19.5 16.4923 18.8972 16.5531 18.1163L17.3788 7.5L6.6212 7.5Z" fill="currentColor" />
                      </svg>
                      ...
                      <div className="cb-file-uploader">
                        <input id="fileCopyOfCharter" type="file" className="cb-file-uploader__visually-hidden" onChange={handleFileChangeCopyOfCharter} multiple />
                        <p className="cb-file-uploader__text">Перетащите файл или выберите на компьютере</p>
                        <a href="" className="cb-file-uploader__link cb-file-uploader__link--make-photo">Сделать фото</a>
                        <label htmlFor="fileCopyOfCharter" className="cb-file-uploader__label">
                          <a href="" className="cb-file-uploader__link">Выбрать файл</a>
                        </label>
                      </div>
                    </div>
                ...
                <div className="cb-spoiler is-toggle">
                  <div className="cb-spoiler__field cb-spoiler__field--substrate">
                    <div className="cb-spoiler__value">
                      ...
                      <p className="cb-spoiler__input">Сведения и документы на право использования и распространения ЦП</p>
                    </div>
                    <div className="cb-spoiler__body">
                      <div className="constructor__row">
                        {/* <span className="cb-icon cb-icon__size-48">
                      <svg width="48" height="24" viewBox="0 0 48 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <a href="#" className="constructor__file" >{fileLawOfUse && `${fileLawOfUse.name}`}</a>
                        <span className="cb-icon cb-icon__size-24">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" clipRule="evenodd" d="M7.99782 5.33184C8.20421 3.9903 9.35852 3 10.7158 3H13.2842C14.6415 3 15.7958 3.99031 16.0022 5.33184L16.105 5.99996H17.4955H19L19 6H20V7.5H18.8833L18.0486 18.2326C17.9271 19.7945 16.6242 21 15.0576 21H8.94239C7.37576 21 6.07291 19.7945 5.95143 18.2326L5.11667 7.5H4V6H5L5 5.99996H6.50453H7.89503L7.99782 5.33184ZM14.5873 5.99996H9.41268L9.48037 5.55993C9.57419 4.95014 10.0989 4.5 10.7158 4.5H13.2842C13.9011 4.5 14.4258 4.95014 14.5196 5.55993L14.5873 5.99996ZM6.6212 7.5L7.44691 18.1163C7.50765 18.8972 8.15908 19.5 8.94239 19.5H15.0576C15.8409 19.5 16.4923 18.8972 16.5531 18.1163L17.3788 7.5L6.6212 7.5Z" fill="currentColor" />
                      </svg>
                      ...
                      <div className="cb-file-uploader">
                        <input id="fileLawOfUse" type="file" className="cb-file-uploader__visually-hidden" onChange={handleFileChangeLawOfUse} multiple />
                        <p className="cb-file-uploader__text">Перетащите файл или выберите на компьютере</p>
                        <a href="" className="cb-file-uploader__link cb-file-uploader__link--make-photo">Сделать фото</a>
                        <label htmlFor="fileLawOfUse" className="cb-file-uploader__label">
                          <a href="" className="cb-file-uploader__link">Выбрать файл</a>
                        </label>
                      </div>
                    </div>
                ...
                <div className="cb-spoiler">
                  <div className="cb-spoiler__field cb-spoiler__field--substrate">
                    <div className="cb-spoiler__value">
                      ...
                      <p className="cb-spoiler__input">Лицензионная политика поставщика</p>
                    </div>
                    <div className="cb-spoiler__body">
                      <div className="constructor__row">
                        {/* <span className="cb-icon cb-icon__size-48">
                      <svg width="48" height="24" viewBox="0 0 48 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <a href="#" className="constructor__file" >{fileLicense && `${fileLicense.name}`}</a>
                        <span className="cb-icon cb-icon__size-24">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" clipRule="evenodd" d="M7.99782 5.33184C8.20421 3.9903 9.35852 3 10.7158 3H13.2842C14.6415 3 15.7958 3.99031 16.0022 5.33184L16.105 5.99996H17.4955H19L19 6H20V7.5H18.8833L18.0486 18.2326C17.9271 19.7945 16.6242 21 15.0576 21H8.94239C7.37576 21 6.07291 19.7945 5.95143 18.2326L5.11667 7.5H4V6H5L5 5.99996H6.50453H7.89503L7.99782 5.33184ZM14.5873 5.99996H9.41268L9.48037 5.55993C9.57419 4.95014 10.0989 4.5 10.7158 4.5H13.2842C13.9011 4.5 14.4258 4.95014 14.5196 5.55993L14.5873 5.99996ZM6.6212 7.5L7.44691 18.1163C7.50765 18.8972 8.15908 19.5 8.94239 19.5H15.0576C15.8409 19.5 16.4923 18.8972 16.5531 18.1163L17.3788 7.5L6.6212 7.5Z" fill="currentColor" />
                      </svg>
                      ...
                      <div className="cb-file-uploader">
                        <input id="fileLicense" type="file" className="cb-file-uploader__visually-hidden" onChange={handleFileChangeLicense} multiple />
                        <p className="cb-file-uploader__text">Перетащите файл или выберите на компьютере</p>
                        <a href="" className="cb-file-uploader__link cb-file-uploader__link--make-photo">Сделать фото</a>
                        <label htmlFor="fileLicense" className="cb-file-uploader__label">
                          <a href="" className="cb-file-uploader__link">Выбрать файл</a>
                        </label>
                      </div>
                    </div>
              ...
              <button id="ProdDocsNextButton" className="cb-button" onClick={onclick}>Далее</button>
            </div>
          </div>
        </div>
      </div>
