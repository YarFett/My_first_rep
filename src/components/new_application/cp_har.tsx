...
import { getItemFromLocalStorage, setItemToLocalStorage } from "./utils";
    ...
    function CpHar(){
        ...
        const [fileDes, setFileDes] = useState<File>();

        useEffect(() => {
            const saveddescription = getItemFromLocalStorage('description');
        ...
        const handleInputDescription = (e: { target: { value: React.SetStateAction<string>; }; }) => {
            ...
            if (e.target.files){
                ...
                reader.onload = () => {
                   const newFile = new Uint8Array(reader.result as ArrayBuffer);
                    setItemToLocalStorage('fileDesData', Array.from(newFile));
                    console.log(newFile)
                }
            setFileDes(e.target.files[0]);
                }
            };
        ...
        const onclick = (e:any) => {
            ...
            data_files.append('statement', fileDes)
            
            const newAnswer= [
                {characteristics: 'Характеристики ЦП'},
                [{description: description},
                {fileDes: fileDes.name}
                ]
                ];
                setAnswer((prevAnswers: any) =>[...prevAnswers, newAnswer]);
                setItemToLocalStorage('description', description)
                setItemToLocalStorage('fileDes', fileDes.name)
                setItemToLocalStorage('fileDesType', fileDes.type)
                history("/cp_info/", {state: newAnswer});}
        ...
        return(
            ...
            <div className="constructor">
                <header className="constructor__header">
                    <a href="" className="constructor__link-back"><Link id="GoBack" to={"/reg_ros_po_yes"}>Назад</Link></a>
                    <p className="constructor__title">Характеристики ЦП</p>
                </header>
                <div className="constructor__inner">
                    ...
                    <div className="constructor__card-item">
                        ...
                        <div className="cb-textarea">
                            <label className="cb-textarea__label">Описание характеристик и функциональных возможностей</label>
                            <div className="cb-textarea__field">
                                <textarea id="HaracteristicsTextArea" className="cb-textarea__input" value={description} onChange={handleInputDescription}></textarea>
                    ...
                    <div className="constructor__row">
                            ...
                            <a href="#" className="constructor__file">{fileDes && `${fileDes.name}`}</a>
                        ...
                        <div className="cb-file-uploader">
                            <input id="cb-file-uploader__field" className="cb-file-uploader__visually-hidden" type="file" onChange={handleFileChange} multiple/>
                    ...
                    <button id="CpHarNextButton" className="cb-button" onClick={onclick}>Далее</button>
                </div>
                </div>
            </div>
            </div>
