...
function CpTrueCOnnect(){
        
    const history = useNavigate();

    const onclickYes = (e: any) => {
        setItemToLocalStorage('cp_true_connect', true)
        history("/dev_gost/", {});

    }
    const onclickNo = (e: any) => {
        setItemToLocalStorage('cp_true_connect', false)
        history("/dev_gost/", {});

    }

    const onclickNotApp = (e: any) => {
        setItemToLocalStorage('cp_true_connect', null)
        history("/dev_gost/", {});
    ...
    return(
        ...
        <div className="constructor">
            <header className="constructor__header">
                ...
                {/* <a href="" className="constructor__link-back"><Link to={"/cp_gost_tls"}>Назад</Link></a> */}
                <p className="constructor__title">ЦП соответствует требованию по обеспечению подлинности сетевых соединений?</p>
                <p className="constructor__card-row-text">В том числе для защиты от подмены сетевых устройств и сервисов</p>
            </header>
